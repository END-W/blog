...
article.Init = function ($) {
    ...
    function blogtype() {
        $('#category').on("mouseover mouseout", "li", function (event) {
    ...
    $.get('/blog/nav/list', function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            $('#category').append("<li data-index=\"" + index + "\"><a href=\"/blog/nav/" + data[i].cname + "/" + data[i].cid + "\">" + data[i].cname + "</a></li>");
            $('.article-category').append("<a href=\"/blog/nav/" + data[i].cname + "/" + data[i].cid + "\">" + data[i].cname + "</a>");
            index++;
        }
    }, 'json');
};
